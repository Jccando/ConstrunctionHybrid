"""E6 - large-scale public-building design-stage cost (ComStock real design features + DDC unit-price knowledge to derive cost).
ComStock public buildings (school/office/hospital/outpatient/hotel/retail) real sqft/HVAC/vintage/climate/stories
  + unit costs anchored by the DDC cost-knowledge base -> resource-based derived total cost (with realistic variability noise).
Value: a third dataset; real design features (incl. sqft, filling the NYC SCA gap); large scale (GPR does not scale, hybrid advantage);
  SHAP yields design-stage cost drivers (area/type/HVAC/vintage). Ablation: drop sqft/HVAC to see the impact.
"""
...
# unit costs (USD/sqft, from DDC China unit prices converted via PPP + anchored to US public-building cost literature, base year 2024)
...
def run_suite(X, y, label):
    ...
    print(f"Stacking vs best single ({bs}): Wilcoxon p={p:.2e}", flush=True)
    return dfr, per


def main():
    print("E6 | ComStock public buildings + DDC-anchored derived cost", flush=True)
    X, y, feats, cs = build_xy()
    print(f"  n_samples={len(y)} n_features={X.shape[1]} median cost=${np.expm1(np.median(y)):,.0f}", flush=True)
    dfr, per = run_suite(X, y, "all features")
    dfr.to_csv(TAB / "e6_results.csv", index=False, encoding="utf-8-sig")

    # component ablation: drop sqft / HVAC / vintage
    print("\n--- component/feature ablation (Stacking) ---", flush=True)
    abl = []
    ...
    # SHAP (full-feature Stacking unsupported; use XGBoost to explain design drivers)
    print("\n--- SHAP design-feature cost drivers ---", flush=True)
    import shap
    ...
    print("\nsaved: e6_results.csv, e6_ablation.csv, e6_shap.csv", flush=True)
