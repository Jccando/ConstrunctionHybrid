"""E5b - RAG retrieval-augmented cost estimation (route-B upgrade: the real innovation).
For each NYC project description, retrieve the top-k most relevant work items from the DDC work-item base (2681 entries),
aggregating their unit price / similarity / category diversity -> rich "contextual cost-knowledge" features fed into the model.
Ablation: M0 (metadata) -> M1 (+work-scope keywords) -> M2_RAG (+RAG retrieval features).
Shows: retrieval-based knowledge augmentation > keyword augmentation (true knowledge-base utilization).
"""
...
def rag_features(nyc_desc, ddc):
    """Retrieve top-K for each description in the DDC base, return a DataFrame of RAG features."""
    ...
    top = np.argpartition(-sim, K, axis=1)[:, :K]    # per-row top-K indices
...
def main():
    ...
    print("E5b | RAG retrieval augmentation (retrieve the DDC work-item base for each project description)", flush=True)
    rag, maxsim = rag_features(df_f["Project Description"].astype(str), ddc)
    print(f"  n_samples={len(df_f)}  DDC_base={len(ddc)}  top-K={K}  max retrieval similarity={maxsim:.3f}", flush=True)
    print(f"  RAG features: {list(rag.columns)}", flush=True)
    ...
    print(f"\nRAG average MAPE reduction {gain:+.2f} percentage points", flush=True)
    ...
    print("saved: e5b_rag_ablation.csv, e5b_rag_ablation.png", flush=True)
