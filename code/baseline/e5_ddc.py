"""E5 - cost-knowledge augmentation ablation ladder (honest version).
On NYC SCA phase-level real cost, inject knowledge layer by layer:
  M0 metadata (program/phase/district/status/year/duration/n_phases)
  M1 + work-scope knowledge (24 work-class keywords guided by the DDC taxonomy, extracted from the description)
  M2 + DDC cost-knowledge-base unit prices (median unit price of matched work classes -> cost-intensity features)
Models: CatBoost / Stacking(+ANN) hybrid. Demonstrates that injecting construction domain knowledge (work scope) substantially improves cost estimation.
"""
...
def main():
    ...
    print("E5 | cost-knowledge augmentation ablation ladder (DDC unit price CNY):", {k: round(v,1) for k,v in prices.items() if v>0}, flush=True)
    DATA = {lvl: build(df, prices, lvl) for lvl in [0, 1, 2]}
    print(f"  n={len(DATA[0][1])}  n_features M0={DATA[0][0].shape[1]} M1={DATA[1][0].shape[1]} M2={DATA[2][0].shape[1]}", flush=True)
    ...
    for mname, mbuild in models:
        ...
        # significance: knowledge injection M0->M1, M1->M2
    ...
    x = np.arange(len(dfr)); w = 0.27; labels = ["M0 metadata", "M1 +work-scope knowledge", "M2 +DDC unit price"]
    ...
    print("\nsaved: tables/e5_ablation.csv, figures/e5_ddc_ablation.png", flush=True)
