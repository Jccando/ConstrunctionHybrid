"""E2 - NYC SCA public-building (school) real cost (phase level).
Y = phase actual spend (Total Phase Actual Spending; real USD, filtered >= $10k to remove near-zero rows).
Features: program type/phase/district/status/year/duration/n_phases + work-scope keywords.
Single (RF/LightGBM/XGBoost/CatBoost/ANN/NGBoost) vs hybrid (Stacking/GA-ANN).
ANN uses TransformedTargetRegressor to standardize y + early_stopping to prevent divergence.
"""
...
def m_ann(s):
    # built-in X standardization + y standardization (TransformedTargetRegressor) + early stopping, to prevent divergence
...
def main():
    ...
    print(f"E2(v3) | NYC SCA phase-level real cost (spend>=$10k)  n={len(y)}  n_features={X.shape[1]}  seeds={N_SEEDS}", flush=True)
    print(f"    Y(phase actual spend) median=${np.expm1(np.median(y)):,.0f}", flush=True)
    per, ae_st = {}, {}
    ...
    print("\n===== E2(v3) results (sorted by MAPE ascending) =====",flush=True)
    print(dfr[["Model","Type","MAPE_str","R2log_str"]].to_string(index=False),flush=True)
    print(f"\nbest hybrid={bh} vs best single={bs}: pooled Wilcoxon p={p_pool:.2e} (n={len(pooled(bh))})",flush=True)

    import matplotlib; matplotlib.use("Agg"); import matplotlib.pyplot as plt
    order=dfr.sort_values("MAPE_mean")
    colors=["#2980b9" if t=="Hybrid" else ("#16a085" if "(new)" in m else "#95a5a6") for m,t in zip(order["Model"],order["Type"])]
    ...
    print("\nsaved: tables/e2_results.csv, figures/e2_mape.png",flush=True)
