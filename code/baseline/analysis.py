"""Combined analysis: (1) Stacking component ablation (each base learner's contribution);
(2) cross-dataset average rank + heatmap + Nemenyi CD diagram;
(3) publication-grade visualization revamp (SHAP beeswarm). Reads e1/e2/e6 result CSVs.
"""
import sys, warnings
...
def component_ablation_uci():
    """On UCI: Stacking (full) vs removing each base learner."""
    ...
    for cname, bases in configs.items():
        ...
        print(f"  component ablation {cname:24s} R2={np.mean(r2s):.4f}±{np.std(r2s):.4f}", flush=True)
...
def cross_dataset():
    """Read e1/e2/e6, cross-dataset average rank + heatmap + CD diagram."""
    def parse_mean(s):
    ...
    def getr(df, model, col):
        m = df[df["Model"].str.replace("(new)", "", regex=False).str.strip() == model]
    ...
    print("\n[cross-dataset performance R2]\n", perf.round(3).to_string(), flush=True)
    # average rank (per dataset, rank 1..k by descending R^2)
...
def shap_beeswarm_uci():
    """UCI SHAP beeswarm (publication-grade, more informative than a bar chart)."""
    ...
    print("  generated SHAP beeswarm", flush=True)


def main():
    print("=== (1) Stacking component ablation (UCI) ===  [done, skip re-run]", flush=True)
    # component_ablation_uci()  # already generated component_ablation.csv/.png
    print("\n=== (2) Cross-dataset summary (rank/heatmap/CD) ===", flush=True)
    cross_dataset()
    print("\n=== (3) SHAP beeswarm (UCI) ===", flush=True)
    shap_beeswarm_uci()
    print("\ngenerated: component_ablation.png, cross_dataset_heatmap.png, cd_diagram.png, e4_shap_beeswarm.png", flush=True)
