"""E1 - UCI #437 method-validation study (v3, expanded method set).
Single (grid-SVR/RF/LightGBM/XGBoost/CatBoost/ANN/NGBoost/GPR) vs hybrid (Stacking/PSO-SVR/GA-ANN).
Target V-10 actual construction cost. 5 seeds -> mean±std + pooled Wilcoxon + probabilistic interval coverage (NGBoost/GPR).
"""
...
# catboost / ngboost are lazily imported inside functions: avoids the gmpy2 DLL crash when joblib worker subprocesses re-import
...
ROOT = Path(__file__).resolve().parents[2]  # project root (baseline/ -> parents[2])
...
NEWER = {"CatBoost", "NGBoost", "GPR"}  # newer-method markers


def main():
    d = load_uci(); X, y = d["X"], d["y_cost"]
    print(f"E1(v3) | UCI #437  n={len(y)}  target=V-10 construction cost  seeds={N_SEEDS}  n_models={len(MODELS)}", flush=True)
    ...
    print("\n===== E1(v3) results (sorted by MAPE ascending) =====", flush=True)
    print(df[["Model", "Type", "MAPE_str", "R2_str", "RMSE_str"]].to_string(index=False), flush=True)
    print(f"\nbest hybrid={best_hybrid} vs best single={best_single}: pooled Wilcoxon p={p_pool:.2e} "
          f"(n={len(pooled(best_hybrid))}); per-seed significant {sig}/{N_SEEDS} (median p={np.median(ps_seed):.2e})", flush=True)
    # probabilistic interval coverage (NGBoost/GPR)
    for nm in ["NGBoost", "GPR"]:
        ...
        if covs:
            ...
            print(f"  {nm}: 90% interval coverage={mc*100:.1f}%  mean interval width={mw:.1f}", flush=True)

    import matplotlib; matplotlib.use("Agg"); import matplotlib.pyplot as plt
    order = df.sort_values("MAPE_mean")
    colors = ["#2980b9" if t == "Hybrid" else ("#16a085" if "(new)" in m else "#95a5a6")
    ...
    print("\nsaved: tables/e1_results.csv, figures/e1_mape.png", flush=True)
