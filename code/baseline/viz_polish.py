"""Visualization revamp (publication aesthetic): read the e1/e2/e6 result CSVs, redraw the MAPE comparison figures.
- unified palette (despined, no chartjunk, error bars, value labels, type-based coloring)
- combined 3-dataset panel (candidate for the paper's main figure)
"""
import sys, warnings
...
def load(df, mape_col):
    df = df.copy()
    df["Model"] = df["Model"].astype(str).str.replace("(new)", "", regex=False).str.strip()
...
def color_for(row):
    ...
    if "(new)" in str(row.get("_orig", "")): return C_NEW
    return C_TRAD


def draw(ax, df, title, ylabel=True):
    colors = [C_HYBRID if t == "Hybrid" else C_NEW if "(new)" in str(m) else C_TRAD
...
def main():
    ...
    print("generated: main_comparison_3panel.png, e1/e2/e6_mape_pub.png", flush=True)
