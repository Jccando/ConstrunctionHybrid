"""E4b - public-building (school) real cost-driver analysis (SHAP), on NYC SCA phase-level real cost.
Features = metadata + work-scope + DDC. SHAP reveals which work classes/phases/districts drive real cost (non-circular, real Y).
"""
import sys, warnings
...
def main():
    ...
    print(f"E4b | NYC SCA SHAP  n={len(y)}  n_features={X.shape[1]}  R2log={m.score(Xte,yte):.3f}", flush=True)
    ...
    print("\nTop 20 real cost drivers (NYC public schools):", flush=True)
    ...
    ax.set_xlabel("mean |SHAP value| (log-cost space)")
    ...
    print("\nsaved: tables/e4b_nyc_shap.csv, figures/e4b_nyc_shap_bar.png", flush=True)
