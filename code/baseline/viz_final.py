"""Publication-grade visualization (dataviz conventions: validated palette + mark specs + story-highlighted series).
Redraws the main figures: 3-dataset MAPE main figure / cross-dataset heatmap / CD diagram / knowledge ablation / component ablation / SHAP drivers.
Palette: hybrid=blue (the story) · newer=orange · traditional=gray (recessive) · sequential blue ramp · blue<->red diverging.
"""
import sys, warnings
...
# ---- validated palette (dataviz conventions) ----
SURF, INK, INK2, MUTED, GRID, AXIS = "#fcfcfb", "#0b0b0b", "#52514e", "#898781", "#e1e0d9", "#c3c2b7"
BLUE, ORANGE, GRAY = "#2a78d6", "#eb6834", "#c8c6be"   # hybrid / newer / traditional(recessive)
SEQ3 = ["#cde2fb", "#86b6ef", "#2a78d6"]               # M0->M2 sequence (increasing knowledge)
...
def classify(model, typecol):
    ...
    if "(new)" in model or model in {"CatBoost", "NGBoost", "GPR"}: return ORANGE
    return GRAY
...
def load_bar(csv, mape_col):
    ...
    df["color"] = [classify(str(m).replace("(new)", "").strip(), t) for m, t in zip(df["Model"], df.get("Type", "Single"))]
    df["model_short"] = df["Model"].str.replace("(new)", "*", regex=False).str.strip()
    return df.sort_values("mape").reset_index(drop=True)


# ============ Fig 1: 3-panel main comparison ============
def fig_main():
    ...
    for ax, (csv, col, title) in zip(axes, dsets):
        ...
        # annotate only the lowest (the hybrid story) and the highest
...
# ============ Fig 4: knowledge ablation (M0/M1/M2) ============
def fig_e5():
...
# ============ Fig 6: SHAP drivers (UCI + NYC, two panels) ============
def fig_shap():
    ...
    for ax, (csv, title) in zip(axes, [("e4_shap_importance.csv", "(a) UCI #437 — residential cost drivers"),
        ...
        cols = [BLUE if i == len(df) - 1 else "#9ec5f4" for i in range(len(df))]  # top (most important) = dark blue
...
def main():
    ...
    print("publication-grade figures generated", flush=True)
